...
def generate_zone(zone_name, environment, zone_type, output_dir, config):
    ...
    if zone_type == 'app':
        ...
        template_vars.update({
            ...
            'geo_routing': True,
            'geo_regions': [
                {'name': 'us-west', 'description': 'West Coast USA', 'server': 'cdn-6.runonflux.io', 'ip': '107.152.47.137'},
                {'name': 'eu-west', 'description': 'France EU', 'server': 'cdn-1.runonflux.io', 'ip': '5.39.57.50'},
                {'name': 'asia-east', 'description': 'Hong Kong Asia', 'server': 'cdn-4.runonflux.io', 'ip': '114.29.237.116'},
            ]
        })
...
def main():
    parser = argparse.ArgumentParser(description='Generate PowerDNS zone files from templates')
    parser.add_argument('zone_name', help='Domain name for the zone (e.g., example.com)')
    parser.add_argument('environment', choices=['staging', 'production'], help='Environment')
    parser.add_argument('zone_type', choices=['app', 'geo', 'simple'], 
                       help='Zone type: app=application routing, geo=geographic routing, simple=basic zone')
    parser.add_argument('-o', '--output-dir', help='Output directory for zone files (default: ../zones from script location)')
    parser.add_argument('-c', '--config', help='Configuration file path (default: ../vars.yaml from script location)')
    parser.add_argument('--dry-run', action='store_true', help='Show template output without writing file')
    ...
    if args.dry_run:
        ...
        generate_zone(args.zone_name, args.environment, args.zone_type, output_dir, config)
